...
const CURRENT_SEASON = [
  { league: 'FLL', challenge: 'BIOGLOW™', status: 'Lançada em 4 ago. 2026', accent: '#FF8A3D' },
  { league: 'FTC', challenge: 'BIOBUZZ™', status: 'Kickoff em 12 set. 2026', accent: '#58AEEA' },
  { league: 'FRC', challenge: 'BIOCORE™', status: 'Revelação em 9 jan. 2027', accent: '#FF4D52' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-end mb-10">
          <div className="lg:col-span-8 space-y-4">
            <div className="inline-flex items-center gap-2 text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-blue-400">
              <span className="w-2 h-2 rounded-full bg-[#ED1C24]" />
              09 / TEMPORADA FIRST
            </div>
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black uppercase tracking-[-0.04em] leading-[0.98]">
              DO KICKOFF<br />AO <span className="text-[#ED1C24]">CHAMPIONSHIP.</span>
            </h2>
          </div>
          <div className="lg:col-span-4">
            <p className="text-base text-gray-400 leading-relaxed">
              Uma temporada é uma corrida contra o calendário: entender o jogo, projetar, construir, programar, testar e chegar pronto para a arena.
            </p>
          </div>
        </div>

        <div className="mb-10 rounded-[26px] border border-white/10 bg-white/[0.045] overflow-hidden">
          <div className="flex flex-col gap-3 border-b border-white/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2 text-[10px] font-mono font-bold uppercase tracking-[0.18em] text-emerald-300">
              <Leaf className="w-4 h-4" />
              FIRST CANOPY™ • 2026–2027
            </div>
            <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500">Tema global: biodiversidade</div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3">
            {CURRENT_SEASON.map((item, index) => (
              <div key={item.league} className={`px-5 py-5 ${index > 0 ? 'border-t sm:border-t-0 sm:border-l' : ''} border-white/10`}>
                <div className="flex items-center justify-between gap-3 mb-2">
                  <span className="text-xs font-black" style={{ color: item.accent }}>{item.league}</span>
                  <span className="w-2 h-2 rounded-full" style={{ backgroundColor: item.accent }} />
                </div>
                <div className="text-lg font-black uppercase tracking-tight text-white">{item.challenge}</div>
                <div className="mt-1 text-[10px] font-mono uppercase tracking-wide text-gray-500">{item.status}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between text-[10px] font-mono font-bold uppercase tracking-[0.18em] text-gray-500 mb-3">
            <span>KICKOFF</span><span>BUILD SEASON</span><span>COMPETIÇÃO</span>
          </div>
            ...
            {SEASON_STEPS.map((item, index) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-5 py-4 border-b last:border-b-0 border-white/10 transition-all flex items-center gap-4 group ${isActive ? 'bg-white text-gray-950' : 'hover:bg-white/5 text-white'}`}
                >
                  <span className={`text-xs font-mono font-bold shrink-0 ${isActive ? 'text-[#ED1C24]' : 'text-gray-600'}`}>{item.number}</span>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-black uppercase tracking-tight truncate">{item.title}</div>
                    <div className="text-[10px] font-mono uppercase mt-1 text-gray-500">{item.period}</div>
                  </div>
              ...
              >
                <div className="relative rounded-[28px] overflow-hidden min-h-[320px] sm:min-h-[410px] border border-white/10 bg-black">
                  ...
                  <div className="absolute top-5 left-5 right-5 flex items-center justify-between gap-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/65 backdrop-blur-md border border-white/15 text-[10px] font-mono font-bold uppercase tracking-wider">
                      <Clock3 className="w-3.5 h-3.5 text-blue-400" />{step.period}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {step.deliverables.slice(0, 3).map((deliverable, index) => (
                    <div key={index} className="p-4 rounded-2xl border border-white/10 bg-white/5 flex items-start gap-3">
                      <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                      <p className="text-xs text-gray-300 leading-relaxed">{deliverable}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
        ...
        <div className="mt-8 p-5 rounded-2xl border border-white/10 bg-white/5 flex items-center justify-between gap-4">
          <div>
            <div className="text-[10px] font-mono font-bold uppercase tracking-[0.18em] text-gray-500 mb-1">CLÍMAX DA TEMPORADA</div>
            <div className="text-sm font-black uppercase">Entregar tudo na arena</div>
          </div>

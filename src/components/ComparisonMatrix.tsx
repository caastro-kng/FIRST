...
import { COMPARISON_CRITERIA, LEAGUES_DATA } from '../data/firstData';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-end mb-14">
          <div className="lg:col-span-8 space-y-4">
            <div className="inline-flex items-center gap-2 text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-[#0066B3]">
              <span className="w-2 h-2 rounded-full bg-[#0066B3]" />04 / COMPARATIVO
            </div>
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black uppercase tracking-[-0.04em] leading-[0.98] text-gray-950">
              TRÊS LIGAS.<br /><span className="text-[#0066B3]">TRÊS ESCALAS.</span>
            </h2>
          </div>
          <div className="lg:col-span-4 lg:pb-1">
            <p className="text-base text-gray-600 leading-relaxed">A tecnologia muda de escala, mas a lógica permanece: projetar, programar, testar e competir em equipe.</p>
          </div>
        </div>
            ...
            return (
              <div key={league.key} className="rounded-[26px] border border-gray-200 bg-white overflow-hidden">
                <div className="p-6 sm:p-7" style={{ backgroundColor: league.soft }}>
                  <div className="flex items-start justify-between gap-4 mb-10">
                    <div>
                      ...
                      <div className="text-sm font-mono text-gray-500 mt-1">{data.targetAge}</div>
                    </div>
                    <div className="w-11 h-11 rounded-2xl bg-white/80 flex items-center justify-center" style={{ color: league.accent }}><Icon className="w-5 h-5" /></div>
                  </div>
                  <div className="space-y-2">
                    <div className="text-4xl font-black tracking-tight" style={{ color: league.accent }}>{data.acronym}</div>
                ...
                <div className="px-6 sm:px-7 py-4 border-t border-gray-200 flex items-center justify-between gap-3">
                  <span className="text-xs font-semibold text-gray-800">{data.name}</span>
        ...
        <div className="rounded-[28px] border border-gray-200 overflow-hidden bg-white">
            ...
            {LEAGUES.map((league) => {
              ...
              return (
                <div key={league.key} className="col-span-3 px-6 py-5 border-l border-white/10">
                  <div className="text-xs font-mono font-bold" style={{ color: league.accent }}>{data.acronym}</div>
                  <div className="text-[10px] text-gray-500 mt-1">{data.targetAge}</div>
                </div>
              );
            })}
          </div>
              ...
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 hover:bg-gray-50/70 transition-colors">
                <div className="md:col-span-3 px-5 sm:px-6 py-5 md:border-r border-gray-100">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-xl bg-gray-100 text-gray-600 flex items-center justify-center shrink-0">{ICONS[item.id] ?? <Check className="w-4 h-4" />}</div>
                    <div>
                  ...
                  return (
                    <div key={league.key} className="md:col-span-3 px-5 sm:px-6 py-4 md:py-5 md:border-l border-gray-100 first:md:border-l-0">
                      <div className="md:hidden flex items-center gap-2 mb-2">
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: league.accent }} />
                        <span className="text-[10px] font-mono font-bold uppercase" style={{ color: league.accent }}>{data.acronym}</span>
                      </div>
          ...
          <div className="px-5 sm:px-6 py-4 bg-[#F8F9FB] border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="text-xs text-gray-500">{showAll ? `Exibindo todas as ${COMPARISON_CRITERIA.length} características.` : 'Exibindo os pontos que mais diferenciam as ligas.'}</div>
            <button onClick={() => setShowAll((prev) => !prev)} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-900 hover:text-[#0066B3] transition-colors">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3">
          {LEAGUES.map((league) => {
            const data = LEAGUES_DATA[league.key];
            return (
              <div key={league.key} className="rounded-2xl border border-gray-200 bg-[#FAFAFA] p-5">
                <div className="text-xs font-mono font-bold mb-2" style={{ color: league.accent }}>{data.acronym}</div>
                <p className="text-sm text-gray-600 leading-relaxed">{data.tagline}</p>
              </div>
            );
          })}
        </div>

        <div className="mt-4 text-[10px] font-mono uppercase tracking-wider text-gray-400">
          Dados técnicos são centralizados em firstData.ts e devem ser revisados quando os manuais anuais da FIRST forem atualizados.
        </div>
      </div>
    </section>
  );
